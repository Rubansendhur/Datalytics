...
export class UserRoleFetchComponent implements OnInit {
  ...
  toggleDeleteButtons(year: string): void {
    if (this.categorizedUsers[year]) {
      this.categorizedUsers[year].hidden = !this.categorizedUsers[year].hidden;
      this.updateYearHiddenState(year, this.categorizedUsers[year].hidden); // Update MongoDB
    }
  }
  ...
  deleteUser(userId: string): void {
    ...
    this.http.delete<{ message: string }>(deleteUrl).subscribe({
      next: () => {
        this.onYearRangeChange(); // Refresh users list after deletion
  ...
  isDeleteButtonVisible(year: string): boolean {
    return !(this.categorizedUsers[year] && this.categorizedUsers[year].hidden);
  }
